...
export function init() {
  ...
  SETTINGS.register(DFChatArchiveNew.PREF_HIDE_EXPORT, {
    ...
    config: true,
    onChange: (newValue) => {
      if (!newValue) $('#chat-controls .export-log').show();
      else $('#chat-controls .export-log').hide();
    },
  });
  ...
  Hooks.on('renderChatLog', function (chatLog: ChatLog, html: JQuery<HTMLElement>) {
    const archiveButton =
      $(`<a class="button chat-archive" title="${'DF_CHAT_ARCHIVE.ExportButtonTitle'.localize()}">
  ...
  Hooks.on('renderSettings', function (settings: Settings, html: JQuery<HTMLElement>) {
    ...
    archiveManagerHtml.on('click', () => {
      if (
        archiveManager == null ||
        ![Application.RENDER_STATES.RENDERED, Application.RENDER_STATES.RENDERING].includes(
          (<any>archiveManager)._state,
        )
      ) {
  ...
  Hooks.on('closeDFChatArchiveNew', () => {
    archiveNew = null;
  });
  Hooks.on('closeDFChatArchiveManager', () => {
    archiveNew = null;
  });
