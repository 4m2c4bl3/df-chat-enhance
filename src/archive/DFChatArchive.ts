...
export class DFChatArchive {
  ...
  private static get DATA_FOLDER(): FilePicker.SourceType {
    return SETTINGS.get(DFChatArchive.PREF_FOLDER_SOURCE);
  }
  ...
  static registerSettings() {
    SETTINGS.register(this.PREF_LOGS, {
      ...
      default: [],
      onChange: () => {
        if (this._updateListener != null) this._updateListener();
      },
    });
  ...
  static getLogs(): DFChatArchiveEntry[] {
    return SETTINGS.get<DFChatArchiveEntry[]>(this.PREF_LOGS);
  }
  static getArchive(id: number): DFChatArchiveEntry {
  ...
  private static async _generateChatArchiveFile(
    id: number,
    name: string,
    chats: ChatMessage[] | ChatMessageData[],
    visible: boolean,
  ): Promise<DFChatArchiveEntry> {
    // Get the folder path
    ...
    const file = new File([JSON.stringify(chats, null, '')], fileName, {
      type: 'application/json',
    });
    const response: { path?: string; message?: string } = <any>(
  ...
  static async createChatArchive(
    name: string,
    chats: ChatMessage[],
    visible: boolean,
  ): Promise<DFChatArchiveEntry> {
  ...
  static async getArchiveContents(
    ...
    const data = await response
      .json()
      .catch((error) =>
        console.error(`Failed to read JSON for archive ${archive.filepath}\n${error}`),
      );
    if (response.ok) return data as (ChatMessage | ChatMessageData)[];
    else throw new Error('Could not access the archive from server side: ' + archive.filepath);
  }

  static async updateChatArchive(
    archive: DFChatArchiveEntry,
    newChatData?: (ChatMessage | ChatMessageData)[],
  ): Promise<DFChatArchiveEntry> {
    ...
    if (newChatData) {
      const folderPath = SETTINGS.get<string>(this.PREF_FOLDER);
      const file = new File([JSON.stringify(newChatData)], archive.filename, {
        type: 'application/json',
      });
      const response: {
        path?: string;
