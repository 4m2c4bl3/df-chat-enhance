...
export default class ChatHistoryOptimizer {
  ...
  private static _ChatLog_onScrollLog(
    this: ChatLog,
    wrapped: (event: any) => Promise<void>,
    event: any,
  ): Promise<void> {
    ...
    if (messages[c].offsetTop < chatViewTop) {
      for (; c < messages.length; c++) {
        if (
          ...
          (messages[c].offsetTop <= chatViewTop &&
            messages[c].offsetTop + messages[c].offsetHeight >= chatViewTop) ||
          // If the previous message is above the view, and this one is in the view
          (c - 1 >= 0 &&
            messages[c].offsetTop >= chatViewTop &&
            messages[c - 1].offsetTop + messages[c - 1].offsetHeight)
        ) {
          topMessageIndex = c;
    ...
    if (
      topMessageIndex < CONFIG.ChatMessage.batchSize / 4 &&
      messages.length < this.collection.size
    ) {
      // Add Messages
