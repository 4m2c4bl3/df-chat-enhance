...
export default class DFChatEdit {
  ...
  static ready() {
    ...
    libWrapper.register(
      SETTINGS.MOD_NAME,
      'ChatLog.prototype._onChatKeyDown',
      (wrapper: (..._: any) => void, ...args: any) => {
        ...
        if (event.code === 'ArrowUp' && event.ctrlKey) {
          ...
          let messages = <(ChatMessageData & ChatMessage)[]>(
            (<any[]>[...ui.chat.collection.values()])
          );
    ...
    libWrapper.register(
      ...
      function (this: ChatLog, wrapper: AnyFunction, message: string) {
        ...
        if (SETTINGS.get(DFChatEditor.PREF_MARKDOWN)) {
          if (message.trim().startsWith('/')) {
            ...
            switch (token) {
              ...
              case '/w':
                [originalMessage, message] = DFChatEditor.processMarkdown(
                  message.substr(token.length),
                );
  ...
  static appendChatContextMenuOptions(options: ContextMenu.Item[]) {
    options.push({
      ...
      condition: (header) => {
        const chatData = <ChatMessageData & ChatMessage>(
  ...
  static isHTML(str: string): boolean {
    ...
    return Array.from(doc.body.childNodes).some((node) => {
      return (
        ...
        !node.classList.contains('df-edited') &&
  ...
  static processChatMessage(
    chatMessage: ChatMessageData & ChatMessage /*, html: JQuery<HTMLElement>*/,
  ): boolean {
    // If we are catching the render of an archived message
    if (!ui.chat.collection.has(chatMessage.id)) return false;
    // Ignore rolls and other people's messages, unless we are the GM and PREF_GM_ALL is true
    if (
      !SETTINGS.get(PREF_EDIT_ALLOWED) ||
      chatMessage.isRoll ||
