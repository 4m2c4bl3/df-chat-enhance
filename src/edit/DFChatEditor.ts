...
export default class DFChatEditor extends FormApplication {
  ...
  getData(options?: any): any {
    ...
    let selected = '';
    if (this.chatMessage.user) selected = 'user-' + this.chatMessage.user;
    if (this.chatMessage.speaker.token) selected = 'token-' + this.chatMessage.speaker.token;
  ...
  async _updateObject(_event?: any, formData?: { content: string; speaker: string }) {
    ...
    if (
      SETTINGS.get<boolean>(DFChatEdit.PREF_SHOW_EDITED) &&
      data.search(/<p +class="df-edited">/) < 0
    ) {
    ...
    if (formData.speaker.startsWith('user-')) {
      ...
      speaker = ChatMessage.getSpeaker({
        token: game.scenes.viewed.tokens.get(formData.speaker.substring('token-'.length)),
      });
