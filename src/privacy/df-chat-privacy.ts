...
export default class ChatRollPrivacy {
  ...
  private static async _handleChatLogRendering(
    chat: ChatLog,
    ...
    for (let c = 0; c < modes.length; c++) {
      ...
      buttons.push({
        ...
        colour: ChatRollPrivacy.calcColour(
          iconKeys.findIndex((x) => x == rt),
          iconKeys.length,
        ),
      });
    }
    const buttonHtml = $(
      await renderTemplate('modules/df-chat-enhance/templates/privacy-button.hbs', { buttons }),
    );
    ...
    if (!SETTINGS.get('replace-buttons')) return;
    ...
    html.find('#chat-controls div.control-buttons a').each(function () {
      ...
      const button = $(
        `<button class="${classes}" title="${title}" style="${style}">${html}</button>`,
      );
