...
export default class DFChatArchiveViewer extends Application {
  ...
  _renderInner(data: any): Promise<JQuery> {
    return super._renderInner(data).then(async (html: JQuery<HTMLElement>) => {
      ...
      const messageHtml = [];
      this.messages = (
        (await DFChatArchive.getArchiveContents(this.archive)) as ChatMessageData[]
      ).filter(
        (x) =>
          game.user.isGM ||
          x.user === game.userId ||
          x.type !== CONST.CHAT_MESSAGE_TYPES.WHISPER ||
          x.whisper.some((x) => x === game.userId),
      );
      ...
      for (const value of this.messages) {
        ...
        try {
          ...
          if (this.messages.length == 1) html.find('a.message-delete').hide();
          html.find('a.message-delete').on('click', (event: JQuery.ClickEvent) => {
            ...
            if (messageHtml.hasClass('dfal-deleted')) {
              ...
              deletionList.splice(
                deletionList.findIndex((x) => x === messageHtml.attr('data-message-id')),
                1,
              );
            } else {
