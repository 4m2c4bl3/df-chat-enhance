...
export default class DFChatArchiveManager extends Application {
  ...
  getData(options?: any) {
    ...
    if (!game.user.isGM) messages = messages.filter((x) => x.visible);
  ...
  private _subscribeView(element: JQuery) {
    element.on('click', function () {
      ...
      if (DFChatArchiveManager.chatViewers.has(id)) {
        ...
        DFChatArchiveManager.chatViewers.set(
          id,
          new DFChatArchiveViewer(DFChatArchive.getArchive(id), (view) => {
  ...
  private _subscribeDelete(element: JQuery) {
    element.on('click', async function () {
      ...
      await Dialog.confirm({
        ...
        content: 'DF_CHAT_ARCHIVE.ArchiveManager_ConfirmDeleteArchiveContent'
          .localize()
          .replace('{name}', archive.name),
        defaultYes: false,
  ...
  activateListeners(html: JQuery) {
    ...
    if (DFChatArchive.getLogs().length > 0) html.find('p.dfca-no-items').hide();
    html.find('a[data-type="view"]').each((i, element) => {
      this._subscribeView($(element));
    });
    html.find('a[data-type="delete"]').each((i, element) => {
      this._subscribeDelete($(element));
    });
          ...
          await Dialog.confirm({
            ...
            yes: async () => {
              ...
              ui.notifications.info(
                'DF_CHAT_ARCHIVE.ArchiveManager_ConfirmDeleteAllComplete'.localize(),
              );
            },
          });
        },
  ...
  private async _archiveChanged() {
    ...
    if (!game.user.isGM) logs = logs.filter((x) => x.visible);
    ...
    if (archiveContainer[0].children.length == 0) this.element.find('p.dfca-no-items').show();
    else this.element.find('p.dfca-no-items').hide();
  }
}
