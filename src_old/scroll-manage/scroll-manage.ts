...
export default class ScrollManage {
  ...
  static get scrollToBottomIfYouSendMessage(): boolean {
    return SETTINGS.get(this.PREF_SCROLL_IF_YOU);
  }
  ...
  private static register() {
    libWrapper.register(
      SETTINGS.MOD_NAME,
      'ChatLog.prototype.postOne',
      this._ChatLog_postOne,
      'OVERRIDE',
    );
    libWrapper.register(
      SETTINGS.MOD_NAME,
      'ChatLog.prototype.scrollBottom',
      this._ChatLog_scrollBottom,
      'OVERRIDE',
    );
    this._deleteMessageRegistrationID = libWrapperShared.register(
      'ChatLog.prototype.deleteMessage',
      this._ChatLog_deleteMessage,
    );
  }
  private static unregister() {
    ...
    libWrapperShared.unregister(
      'ChatLog.prototype.deleteMessage',
      this._deleteMessageRegistrationID,
    );
  }
  ...
  private static async _ChatLog_postOne(
    this: ChatLogExt,
    message: ChatMessageData & ChatMessage,
    notify = false,
  ) {
    ...
    const atBottom =
      element[0].scrollTop >=
      element[0].scrollHeight - element[0].clientHeight - ScrollManage.ScrollThreshold;
  ...
  private static _ChatLog_deleteMessage(
    this: ChatLogExt,
    wrapped: (...args: any) => unknown,
    messageId: string,
    { deleteAll = false }: { deleteAll?: boolean } = {},
  ): unknown {
