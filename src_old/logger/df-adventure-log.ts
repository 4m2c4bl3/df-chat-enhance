...
export function ready() {
  ...
  Hooks.on(
    'renderUserConfig',
    (_app: UserConfig, html: JQuery<HTMLElement>, data: UserConfig.Data<any>) => {
      const journal = (<string>(
        data.user.getFlag(SETTINGS.MOD_NAME, DFAdventureLogProcessor.PREF_PLAYER_LOG_JOURNAL)
      ))?.split('.');
...
`);
        ...
        template(
          {
            ...
            pages: journal
              ? journals
                  .find((x) => x.id == journal[0])
                  .pages.contents.filter((x) => x.type === 'text')
                  .map((x) => ({
                    id: x.id,
                    name: x.name,
      ...
      journalSelection.find('#dfce-player-log').on('change', (event) => {
        ...
        for (const page of game.journal
          .get(journalId)
          .pages.contents.filter((x) => x.type === 'text')) {
  ...
  libWrapperShared.register(
    'UserConfig.prototype._updateObject',
    async function (
      this: UserConfig,
      wrapped: (...args: any) => any,
      event: any,
      formData: { 'player-log-page': string },
    ) {
      await this.object.setFlag(
        SETTINGS.MOD_NAME,
        DFAdventureLogProcessor.PREF_PLAYER_LOG_JOURNAL,
        formData['player-log-page'],
      );
...
async function migrateDataV9toV10() {
  ...
  if (checkJournal(journalId))
    await SETTINGS.set<string>(
      DFAdventureLogConfig.PREF_JOURNAL,
      await migrateJournalV9toV10(journalId),
    );
  journalId = SETTINGS.get<string>(DFAdventureLogConfig.PREF_JOURNAL_GM);
  if (checkJournal(journalId))
    await SETTINGS.set<string>(
      DFAdventureLogConfig.PREF_JOURNAL_GM,
      await migrateJournalV9toV10(journalId),
    );

  for (const user of game.users) {
    journalId = <string>(
      user.getFlag(SETTINGS.MOD_NAME, DFAdventureLogProcessor.PREF_PLAYER_LOG_JOURNAL)
    );
...
function checkJournal(id: string) {
  return id && id.split('.').length < 2;
}
