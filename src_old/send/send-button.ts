...
export default class SendButton {
    ...
    SETTINGS.register(this.PREF_ENABLED, {
      ...
      onChange: (isActive) => {
        if (isActive) {
          ...
          if (popout.length > 0) this._renderChatLog(ui.chat, popout);
        } else {
  ...
  private static _renderChatLog(app: ChatLog, html: JQuery<HTMLElement>) {
    ...
    const sendButton = $(
      '<button class="dfce-send-btn"><i class="fas fa-paper-plane"></i></button>',
    );
    ...
    sendButton.prop(
      'disabled',
      textarea[0].textLength <= 0 || textarea[0].value.trim().length <= 0,
    );
