...
export default class ChatMerge {
  ...
  private static get _epoch(): number {
    return SETTINGS.get(this.PREF_EPOCH);
  }
  private static get _allowRolls(): string {
    return SETTINGS.get(this.PREF_ALLOW_ROLLS);
  }
  private static get _separateWithBorder(): boolean {
    return SETTINGS.get(this.PREF_SEPARATE);
  }
  private static get _showHover(): boolean {
    return SETTINGS.get(this.PREF_HOVER);
  }
  private static get _showHeader(): boolean {
    return SETTINGS.get(this.PREF_SHOW_HEADER);
  }
  ...
  static ready() {
    ...
    Hooks.on('renderDFChatArchiveViewer', (_: any, html: JQuery<HTMLElement>) =>
      this._processAllMessage(html),
    );
  }

  private static _deleteMessage(
    wrapper: (arg0: any, arg1: any) => any,
    messageId: string,
    { deleteAll = false } = {},
  ) {
  ...
  private static _renderChatMessage(
  ...
  private static _isValidMessage(current: ChatMessageData, previous: ChatMessageData): boolean {
    ...
    if (
      (currData?.flags && (currData.flags as Record<string, any>)['midi-qol']) ||
      (prevData?.flags && (prevData.flags as Record<string, any>)['midi-qol'])
    )
      return false;
  ...
  private static _styleChatMessages(
    curr: ChatMessageData,
    ...
    if (curr === undefined && prev === undefined) {
      ...
      curr = <ChatMessageData>(
        chatLog.messages.find((x) => (x.id ?? x._id) == currElem.dataset.messageId)
      );
      prev = <ChatMessageData>(
        chatLog.messages.find((x) => (x.id ?? x._id) == prevElem.dataset.messageId)
      );
    }
